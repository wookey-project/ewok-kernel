...
sub dump_elf_metainfo {
    ...
    $appinfo{'text_off'} = hex($app_memorymap{'flash_slot_addr'}) - hex($socinfos->{"memory.flash.\L$mode\E.addr"});
    $appinfo{'data_off'} = hex($app_memorymap{'ram_slot_addr'}) - hex($socinfos->{"memory.ram.addr"});
